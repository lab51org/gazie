...
ALTER TABLE `gaz_XXXrigbro`	CHANGE COLUMN `id_doc` `id_doc` INT(9) NOT NULL DEFAULT '0' COMMENT 'può essere usato come riferimento ad un figlio anche se in rigdoc (es. se questo è il rigo di un ordine evaso) o ad un genitore (es. se questo è un task di un diagramma di Gantt anch\'esso in rigbro)' AFTER `delivery_date`;
