...
ALTER TABLE `gaz_XXXdistinta_base` ADD COLUMN `id_movmag` INT(9) UNSIGNED NULL COMMENT 'Riferimento ad id_mag del movimento di magazzino. Se =0 indica la struttura di una composizione e verrà utilizzata per generare i relativi movimenti di magazzino, quando valorizzato serve per ricordare l\'albero da cui sono stati generati i relativi movimenti di magazzino' AFTER `quantita_artico_base`;
