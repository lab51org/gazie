...
ALTER TABLE `gaz_aziend` ADD `virtual_taxstamp` TINYINT NOT NULL AFTER `round_bol`;
UPDATE `gaz_aziend` SET `virtual_taxstamp` = 1 WHERE 1; 
UPDATE `gaz_aziend` SET `template` = '' WHERE 1; 
ALTER TABLE `gaz_aziend` CHANGE `alliva` `preeminent_vat` INT NOT NULL;
ALTER TABLE `gaz_aziend` DROP `iva_susp`;
ALTER TABLE `gaz_aziend` CHANGE `ricbol` `taxstamp` DECIMAL(5,2) NOT NULL;
ALTER TABLE `gaz_aziend` CHANGE `ivabol` `taxstamp_vat` INT NOT NULL;
...
ALTER TABLE `gaz_XXXtesdoc` ADD `virtual_taxstamp` TINYINT NOT NULL AFTER `taxstamp`;
ALTER TABLE `gaz_XXXtesbro` ADD `virtual_taxstamp` TINYINT NOT NULL AFTER `taxstamp`;
