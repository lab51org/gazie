...
ALTER TABLE `gaz_XXXrigbro`	CHANGE COLUMN `id_doc` `id_doc` INT(9) NOT NULL DEFAULT '0' COMMENT 'può essere usato come riferimento ad un figlio anche se in rigdoc (es. se questo è il rigo di un ordine evaso) o ad un genitore (es. se questo è un task di un diagramma di Gantt sarà l\'id_tes in tesbro)' AFTER `delivery_date`;
ALTER TABLE `gaz_XXXcamp_mov_sian` ADD INDEX (`recip_stocc`);
ALTER TABLE `gaz_XXXagenti`	ADD COLUMN `id_agente_coord` INT(9) NOT NULL COMMENT 'per la gestione dei subagenti, questa è la referenza all\'agente coordinatore' AFTER `id_agente`,	ADD INDEX `id_agente_coord` (`id_agente_coord`);
