...
if (empty($form['id_agente']) && empty($form['clifor'])) { // mancano i dati per la selezione
   ...
   $titolo = "FORNITORI DELL'AGENTE: " . queryNomeAgente($form['id_agente'], $gTables);
   ...
   $titolo = "FORNITORI IN ORDINE DI ZONA ";
...
$title = array('luogo_data'=>$luogo_data,
    ...
    'hile' => array(/* array('lun' => 45, 'nam' => 'Cliente'), */
        array('lun' => 60, 'nam' => 'Ragione Sociale / Sede legale'),
...
while ($partner = gaz_dbi_fetch_array($rs)) {
   $pdf->Cell(60, 4, $partner['ragioneSociale'], 'LTR', 0, 'L', true, '', 1);
   $pdf->Cell(70, 4, $partner['sede'], 'T', 0, 'R', true, '', 1);
   $pdf->Cell(65, 4, $partner["telefono"],'LTR', 1, 'C', true, '', 1);
   $pdf->SetFont('helvetica','',7);
   $pdf->Cell(60, 4, $partner['sedleg'], 'LBR', 0, 'L', false, '', 1);
   $pdf->SetFont('helvetica','',9);
   $pdf->Cell(70, 4, $partner["e_mail"], 'B', 0, 'L', false, '', 1);
   $pdf->Cell(65, 4, 'Pagam: '.$partner['payment'], 'LBR', 1, 'R', false, '', 1);
}
