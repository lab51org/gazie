...
if (isset($_GET['term'])) { //	Evitiamo errori se lo script viene chiamato direttamente
    ...
    switch ($opt) {
        ...
        case 'suggest_sea_descri':
            $result = gaz_dbi_dyn_query("descri AS id, CONCAT(id_tesdoc_ref,' ',descri ) AS label, descri AS value, 'S' AS movimentabile",$gTables['paymov']." LEFT JOIN ".$gTables['rigmoc']." ON (".$gTables['paymov'].".id_rigmoc_pay = ".$gTables['rigmoc'].".id_rig OR ".$gTables['paymov'].".id_rigmoc_doc = ".$gTables['rigmoc'].".id_rig) LEFT JOIN ".$gTables['tesmov']." ON ".$gTables['rigmoc'].".id_tes = ".$gTables['tesmov'].".id_tes", "descri LIKE '%".$term."%' AND (CONVERT(SUBSTR(id_tesdoc_ref,5,1),DECIMAL) <= 4 OR id_tesdoc_ref LIKE '%new%')", "expiry DESC");
            break;
