...
//rimossa   $orderby = "ragso1 ASC, pagame ASC, numdoc ASC, id_rig ASC";
   $orderby = "ragso1 ASC, ragbol ASC, pagame ASC, numdoc ASC, id_rig ASC";
//rimossa   $where = "tipdoc = 'DDT' AND datemi BETWEEN '" . $period['inizio'] . "' AND '" . $period['fine'] . "' AND seziva = '$sezione' and da_fatturare=true " . $clientesel;
   $where = "tipdoc = 'DDT' AND datemi BETWEEN '" . $period['inizio'] . "' AND '" . $period['fine'] . "' AND seziva = '$sezione' and da_fatturare=true " . $clientesel;
   /** fine modifica FP */
   ...
   if ($ctrlnum) {
      ...
      while ($row = gaz_dbi_fetch_array($result)) {
         ...
         /** inizio modifica FP 20/10/15 */
//rimossa         if ($row['clfoco'] != $ctrlc or $row['pagame'] != $ctrlp or ( $row['id_tes'] != $ctrld and $row['ragdoc'] == 'N')) {  //se è un'altro cliente o il cliente ha un pagamento diverso dal precedente
...
/** inizio modifica FP 20/10/2015 */
//rimossa   $rs_ddtsez = gaz_dbi_dyn_query("*", $gTables['tesdoc'], "tipdoc = 'DDT' and seziva = $sezione", "datemi asc", 0, 1);
$rs_ddtsez = gaz_dbi_dyn_query("*", $gTables['tesdoc'], "tipdoc = 'DDT' and seziva = $sezione and da_fatturare=true ", "datemi asc", 0, 1);
/** fine modifica FP */
        ...
        <tr>
            ...
            <td class="FacetDataTD" >
                ...
                if (strlen($_POST['ragso1']) >= 2) {
                   ...
                   $result = gaz_dbi_dyn_query("*", $gTables['clfoco'] . ' LEFT JOIN ' . $gTables['anagra'] . ' ON ' . $gTables['clfoco'] . '.id_anagra = ' . $gTables['anagra'] . '.id', "codice like '$clienti%' and codice > '$mascon'  and ragso1 like '" . addslashes($cerca) . "%'", "ragso1 DESC");
