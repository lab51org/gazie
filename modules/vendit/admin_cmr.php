...
if ((isset($_POST['Insert'])) or ( isset($_POST['Update']))) {   //se non e' il primo accesso
    ...
    if (isset($_POST['ins'])) {
        ...
        if ($toDo == 'update') {  // controlli in caso di modifica
            $rs_query = gaz_dbi_dyn_query("numdoc", $gTables['tescmr'], "YEAR(datemi) = " . $form['annemi'] . " and datemi < '$datemi' and tipdoc = '" . $form['tipdoc'] . "' and seziva = $sezione", "datemi DESC, numdoc DESC", 0, 1);
            ...
            $rs_ultimo_tipo = gaz_dbi_dyn_query("*", $gTables['tescmr'], "YEAR(datemi) = " . $form['annemi'] . " and tipdoc = '" . $form['tipdoc'] . "' and seziva = $sezione", "numdoc desc, datemi desc", 0, 1);
        ...
        if ($msg == "") {// nessun errore
            ...
            if ($toDo == 'update') { // e' una modifica
                $old_rows = gaz_dbi_dyn_query("*", $gTables['rigcmr'], "id_tes = " . $form['id_tes'], "id_rig asc");
                $i = 0;
                ...
                while ($val_old_row = gaz_dbi_fetch_array($old_rows)) {
                    if ($i <= $count) { //se il vecchio rigo e' ancora presente nel nuovo lo modifico
                        ...
                        rigcmrUpdate($codice, $form['rows'][$i]);
                        if (isset($form["row_$i"]) && $val_old_row['id_body_text'] > 0) { //se � un rigo testo gi� presente lo modifico
                            ...
                            gaz_dbi_put_row($gTables['rigcmr'], 'id_rig', $val_old_row['id_rig'], 'id_body_text', $val_old_row['id_body_text']);
                        } elseif (isset($form["row_$i"]) && $val_old_row['id_body_text'] == 0) { //prima era un rigo diverso da testo
                            bodytextInsert(array('table_name_ref' => 'rigcmr', 'id_ref' => $val_old_row['id_rig'], 'body_text' => $form["row_$i"], 'lang_id' => $admin_aziend['id_language']));
                            gaz_dbi_put_row($gTables['rigcmr'], 'id_rig', $val_old_row['id_rig'], 'id_body_text', gaz_dbi_last_id());
                        } elseif (!isset($form["row_$i"]) && $val_old_row['id_body_text'] > 0) { //un rigo che prima era testo adesso non lo � pi�
                            gaz_dbi_del_row($gTables['body_text'], "table_name_ref = 'rigcmr' AND id_ref", $val_old_row['id_rig']);
                        }
                    } else { //altrimenti lo elimino
                        if (intval($val_old_row['id_body_text']) > 0) {  //se c'� un testo allegato al rigo elimino anch'esso
                            gaz_dbi_del_row($gTables['body_text'], "table_name_ref = 'rigcmr' AND id_ref", $val_old_row['id_rig']);
                        }
                        gaz_dbi_del_row($gTables['rigcmr'], "id_rig", $val_old_row['id_rig']);
                    }
                    $i++;
                }
                ...
                for ($i = $i; $i <= $count; $i++) {
                    $form['rows'][$i]['id_tes'] = $form['id_tes'];
                    rigcmrInsert($form['rows'][$i]);
                    $last_rigbro_id = gaz_dbi_last_id();
                    if (isset($form["row_$i"])) { //se � un rigo testo lo inserisco il contenuto in body_text
                        bodytextInsert(array('table_name_ref' => 'rigcmr', 'id_ref' => $last_rigbro_id, 'body_text' => $form["row_$i"], 'lang_id' => $admin_aziend['id_language']));
                        gaz_dbi_put_row($gTables['rigcmr'], 'id_rig', $last_rigbro_id, 'id_body_text', gaz_dbi_last_id());
                    }
                }
                //modifico la testata con i nuovi dati...
                $old_head = gaz_dbi_get_row($gTables['tescmr'], 'id_tes', $form['id_tes']);
                ...
                $rs_ultimo_documento = gaz_dbi_dyn_query("*", $gTables['tescmr'], $sql_documento, $where, 0, 1);
        ...
        if ($msg == "") {// nessun errore
			...
			$sql_documento = "YEAR(datemi) = " . date("Y") . " and tipdoc = 'CMR'";
            $rs_ultimo_documento = gaz_dbi_dyn_query("*", $gTables['tescmr'], $sql_documento, "numdoc desc", 0, 1);
            $ultimo_documento = gaz_dbi_fetch_array($rs_ultimo_documento);
            if ($ultimo_documento) {
                $form['numdoc'] = $ultimo_documento['numdoc'] + 1;
            } else {
...
//rimossa    $form['in_sconto'] = 0;
    ...
    $tesbro = gaz_dbi_get_row($gTables['tescmr'], "id_tes", $_GET['id_tes']);
    $anagrafica = new Anagrafica();
    $cliente = $anagrafica->getPartner($tesbro['clfoco']);
    $rs_rig = gaz_dbi_dyn_query("*", $gTables['rigcmr'], "id_tes = " . intval($_GET['id_tes']), "id_rig asc");
...
require("./lang." . $admin_aziend['lang'] . ".php");
$script_transl = HeadMain(0, array(/* 'tiny_mce/tiny_mce', */ //$strScript["admin_broven.php"] +
            /* 'boxover/boxover', */
...
/*if ($toDo == 'update' and $form['tipdoc'] == 'CMR') {
    echo '			<tr>
