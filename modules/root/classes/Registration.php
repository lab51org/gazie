...
class Registration
{
    ...
    private function registerNewUser($user_name, $user_email, $user_password, $user_password_repeat, $captcha)
    {
        ...
        // finally if all the above checks are ok
            ...
            $query_check_user_name = $this->db_connection->prepare('SELECT Login, user_email FROM ' . DB_TABLE_PREFIX . '_admin WHERE Login=:user_name OR user_email=:user_email');
            ...
            if (count($result) > 0) {
                for ($i = 0; $i < count($result); $i++) {
                    $this->errors[] = ($result[$i]['Login'] == $user_name) ? MESSAGE_USERNAME_EXISTS : MESSAGE_EMAIL_ALREADY_EXISTS;
                }
            } else {
                ...
                $query_new_user_insert = $this->db_connection->prepare('INSERT INTO ' . DB_TABLE_PREFIX . '_admin (Login, user_password_hash, user_email, user_activation_hash, user_registration_ip, user_registration_datetime) VALUES(:user_name, :user_password_hash, :user_email, :user_activation_hash, :user_registration_ip, NOW())');
                ...
                if ($query_new_user_insert) {
                    ...
                    if ($this->sendVerificationEmail($user_id, $user_email, $user_activation_hash)) {
                        ...
                        $query_delete_user = $this->db_connection->prepare('DELETE FROM ' . DB_TABLE_PREFIX . '_admin WHERE SHA2(Login)=:user_id');
    ...
    public function sendVerificationEmail($user_id, $user_email, $user_activation_hash)
    {
        ...
        $mail->AddEmbeddedImage('./school.png', 'gazie');
    ...
    public function verifyNewUser($user_id, $user_activation_hash)
    {
        ...
        if ($this->databaseConnection()) {
            ...
            $query_update_user = $this->db_connection->prepare('UPDATE ' . DB_TABLE_PREFIX . '_admin SET user_active = 1, user_activation_hash = NULL WHERE SHA2(Login) = :user_id AND user_activation_hash = :user_activation_hash');
            $query_update_user->bindValue(':user_id', trim($user_id), PDO::PARAM_STR);
