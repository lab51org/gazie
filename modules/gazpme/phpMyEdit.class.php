    ...
    function list_table() /* {{{ */
    {
        ...
        for ($k = 0; $k < $this->num_fds; $k++) {
            ...
            if (! $this->fdd[$fd]['sort'] || $this->password($fd)) {
                ...
                // INIZIO patch per GAzie
                echo '<div onclick="GAz_sys_form.'.$this->cgi['prefix']['sys'].'fm.value=0; '.
                                   'GAz_sys_form.'.$this->cgi['prefix']['sys'].'fl.value='.$this->fl.'; '.
                                   'GAz_sys_form.elements[\''.$this->cgi['prefix']['sys'].'sfn[0]\'].value='.$new_sfn[0].'; '.
                                   'GAz_sys_form.submit();'.'" class="',$css_class_name,'" style="cursor:pointer;">';
                echo $fdn;
                echo '</div>';
                // FINE patch per GAzie


/*              --- VECCHIO METODO CESTINATO
                    FRANCAMENTE NON CAPISCO L'UTILITA' DI CREARE UN FILE php PER OGNI TABELLA!
