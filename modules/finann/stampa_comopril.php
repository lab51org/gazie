       ...
       while ($row = gaz_dbi_fetch_array($result)) {
         ...
         if ($ctrl_id <> $row['idtes']) {
            ...
            if (isset($castel_transact[$ctrl_id])
               ...
               if ($admin_aziend['country'] != $row['country']) { //  uno non residente (caso 3)
                     if (!empty($row['datnas'])) { //  un persona fisica straniera
                        if (empty($row['pronas']) || empty($row['luonas']) || empty($row['counas'])) {
                            $error_transact[$row['idtes']][] = $script_transl['errors'][9];
                        }
                     }                
               } elseif (empty($resultpi) && !empty($row['pariva'])) { // ha la partita IVA ed  giusta (caso 2) 
                 if( strlen(trim($row['codfis'])) == 11) { //  una persona giuridica
               ...
               } else {        //  un soggetto con codice fiscale senza partita IVA (caso 1)
                     $resultcf = $nuw->check_TAXcode($row['codfis']);
                     if (empty($row['codfis'])) {
                         $error_transact[$row['idtes']][] = $script_transl['errors'][3];
                     } elseif ($row['sexper'] == 'G' and empty($resultcf)) {
            ...
            if ($row['country'] <> 'IT' ) { // non residente
                ...
                if (!empty($row['pariva'])){ //  una azienda straniera quindi forzo l'eliminazione azzerando i valori
                    $value_imponi = 0;
                    $value_impost = 0;
                }
            } else {
