...
if (isset($_GET['anteprima']) and $msg == "") {
  // promemoria esempio per ricercare mysql in custom field (custom_field REGEXP 'accommodation_type')
    ...
    $where = $gTables['rental_events'].".type = 'ALLOGGIO' AND (".$gTables['rental_events'].".start BETWEEN '$datainizio' AND '$datafine' )";
...
//echo "<pre>",print_r($rows);die;
      ...
      foreach ($rows as $row) {// per ogni prenotazione
        if (isset($row['dayStat']) && $row['dayStat']>0){
           ...
           // while($val_rows[] = mysqli_fetch_assoc($result));array_pop($val_rows);// creo un array con tutti i dati
                      ...
                      foreach ($count[$key] as $key2 => $item2) {
                        ...
                        if ($key2=="pern_tot"){
                          ...
                          foreach ($count[$key]['room_tarif_periodo_pers'] as $key3 => $item3) {
                            switch($key3){
                              case "1":
                                echo "<br>B20- Tariffa <b>monolocale</b> media a persona periodo: € ",gaz_format_number($count[$key]['room_tarif_periodo_pers'][$key3])," (iva compresa)";
                                break;
                              case "2":
                                echo "<br>B21- Tariffa <b>bilocale</b> media a persona periodo: € ",gaz_format_number($count[$key]['room_tarif_periodo_pers'][$key3])," (iva compresa)";
                                break;
                              case "3":
                                echo "<br>B22- Tariffa <b>trilocale</b> media a persona periodo: € ",gaz_format_number($count[$key]['room_tarif_periodo_pers'][$key3])," (iva compresa)";
                                break;
                              case "4":
                                echo "<br>B23- Tariffa <b>quadrilocale</b> media a persona periodo: € ",gaz_format_number($count[$key]['room_tarif_periodo_pers'][$key3])," (iva compresa)";
                                break;
                            }
                          }
                        }else{ // è un mese
                          ...
                          foreach ($count[$key][$key2] as $key4 => $item4){
                            ...
                            if ($key4=="pern_tot"){
                              ...
                              echo "<br>Pernottamenti minori di anni ",$minor,", totali mese: ",$count[$key][$key2]['pern_tot_child'];
