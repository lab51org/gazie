...
class BookingSummary extends Template
{
    function setTesDoc()
    {
        ...
        $this->tipdoc = 'Booking n.'.$this->tesdoc['numdoc'].'/'.$this->tesdoc['seziva'].' of '.$this->giorno.' '.$this->nomemese.' '.$this->anno;
    ...
    function body()
    {
        $lines = $this->docVars->getRigo('english');
    ...
    function pageFooter()
    {
		...
		if ( $this->tesdoc['print_total']>0){
			...
			if ($payed>0){
				...
				$this->Cell(56, 6, 'BALANCE amount', 1, 0, 'C', 1);
				 $this->SetY(233);
				$this->Cell(130);
				$this->SetFont('helvetica','B',18);
    ...
    function Footer()
    {
        ...
        $this->SetFont('helvetica', '', 8);
        if ( $this->sedelegale!="" ) {
            $this->MultiCell(184, 4, $this->intesta1 . ' ' . $this->intesta2 . ' ' . $this->intesta3 . ' ' . $this->intesta4 . ' ' . "REGISTERED OFFICE: ".$this->sedelegale, 0, 'C', 0);
        } else {
