...
<?php
$recordnav = new recordnav($gTables['tescmr'], $where, $limit, $passo);
$recordnav->output();
?>
<form method="GET" >
    ...
    <table class="Tlarge table table-striped table-bordered table-condensed">
        <tr>
            ...
            <td class="FacetFieldCaptionTD">
                <?php
                    gaz_flt_disp_select("datemi", "YEAR(datemi) as datemi", $gTables["tescmr"], $all, $orderby); 
                ?>
            </td>
            <td class="FacetFieldCaptionTD">
                <?php gaz_flt_disp_select("clfoco", $gTables['anagra'] . ".ragso1," . $gTables["tescmr"] . ".clfoco", $gTables['tescmr'] . " LEFT JOIN " . $gTables['clfoco'] . " ON " . $gTables['tescmr'] . ".clfoco = " . $gTables['clfoco'] . ".codice LEFT JOIN " . $gTables['anagra'] . " ON " . $gTables['clfoco'] . ".id_anagra = " . $gTables['anagra'] . ".id", $all, $orderby, "ragso1"); ?>
            </td>
            <td class=FacetFieldCaptionTD>
                <?php gaz_flt_disp_select("unita_locale1", $gTables['destina'].".unita_locale1", $gTables['tescmr'] . " LEFT JOIN " . $gTables['clfoco'] . " ON " . $gTables['tescmr'] . ".clfoco = " . $gTables['clfoco'] . ".codice LEFT JOIN " . $gTables['anagra'] . " ON " . $gTables['clfoco'] . ".id_anagra = " . $gTables['anagra'] . ".id left join ". $gTables['destina']." on " .$gTables['tescmr'].".id_des_same_company = " . $gTables['destina'] . ".codice" , $all, $orderby, "unita_locale1"); ?>
            </td>
            <td class=FacetFieldCaptionTD>
                &nbsp;
            </td>
...
//recupero le testate in base alle scelte impostate
        $result = gaz_dbi_dyn_query($gTables['tescmr'] . ".*," . $gTables['anagra'] . ".ragso1," . $gTables['anagra'] . ".e_mail AS base_mail," . $gTables["clfoco"] . ".codice, ".$gTables["destina"].".*", $gTables['tescmr'] . " LEFT JOIN " . $gTables['clfoco'] . " ON " . $gTables['tescmr'] . ".clfoco = " . $gTables['clfoco'] . ".codice  LEFT JOIN " . $gTables['anagra'] . ' ON ' . $gTables['clfoco'] . '.id_anagra = ' . $gTables['anagra'] . '.id  left join '. $gTables['destina'].' on ' .$gTables['tescmr'].'.id_des_same_company = ' . $gTables['destina'] . '.codice', $where, $orderby, $limit, $passo);
        if ($result == false) {
            die(mysql_error());
        }
        $ctrlprotoc = "";
        while ($r = gaz_dbi_fetch_array($result)) {
            if ($r["tipdoc"] == 'CMR') {
                $modulo = "stampa_cmr.php?id_tes=" . $r['id_tes'];
                $modifi = "admin_cmr.php?Update&id_tes=" . $r['id_tes'];
            }
            echo "<tr class=\"FacetDataTD\">";

            if (!empty($modifi)) {
                echo "<td><a class=\"btn btn-xs btn-default btn-edit\" title=\"" . $script_transl['type_value'][$r["tipdoc"]] . "\" href=\"" . $modifi . "\"><i class=\"glyphicon glyphicon-edit\"></i>&nbsp;" . $r["tipdoc"] . "&nbsp;" . $r["id_tes"] . "</td>";
            } else {
                echo "<td><button class=\"btn btn-xs btn-default disabled\">&nbsp;" . $r["tipdoc"] . "&nbsp;" . $r["id_tes"] . " </button></td>";
            }
            ...
            $rigbro_result = gaz_dbi_dyn_query('*', $gTables['rigcmr'], "id_tes = " . $r["id_tes"] . " AND tiprig <=1 ", 'id_tes DESC');
            while ($rigbro_r = gaz_dbi_fetch_array($rigbro_result)) {
                ...
                if ($rigbro_r["id_doc"] != $tesdoc_r["id_tes"]) {
                    ...
                    //
            gaz_dbi_put_row($gTables['rigcmr'], "id_tes", $rigbro_r["id_tes"], "id_doc", 0);
                    //
    ...
    if ($remains_atleastone && !$processed_atleastone) {
        ...
        if ($r["status"] != "GENERATO") {
                    gaz_dbi_put_row($gTables['tescmr'], "id_tes", $r["id_tes"], "status", "RIGENERATO");
                }
                echo "<td><a class=\"btn btn-xs btn-warning\" href=\"select_evacmr.php?id_tes=" . $r['id_tes'] . "\">evadi</a></td>";
                ...
                // a questa testata.
                //
                $rigbro_result = gaz_dbi_dyn_query('*', $gTables['rigcmr'], "id_tes = " . $r["id_tes"], 'id_tes DESC');
                //
                while ($rigbro_r = gaz_dbi_fetch_array($rigbro_result)) {
                    if ($rigbro_r["id_doc"] == 0) {
                        continue;
                ...
                echo "<a class=\"btn btn-xs btn-default\" href=\"select_evacmr.php?id_tes=" . $r['id_tes'] . "\">evadi il rimanente</a></td>";
            } else {
                echo "<td>";
                //
                $ultimo_documento = 0;
