...
if (isset($_POST['mov']) && isset($_POST['artico' . $_POST['mov']])) {
    ...
    if ($form['artico'][$form['mov']] <> "" && !isset($itemart)) {
        $msg.= "18+";
    } // controllo se codice articolo non corrisponde a quelli forniti da autocomplete o se è nullo
...
if (!isset($_POST['Update']) and isset($_GET['Update'])) { //se è il primo accesso per UPDATE
    ...
    if (intval($form['id_orderman']) > 0) { //se è presente una produzione, carico il campo di coltivazione ad essa collegato e la relativa coltura
        ...
        // Carico il campo in item_campi
		$form['campo_coltivazione'] = $rs_orderman['campo_impianto'];
        $item_campi = gaz_dbi_get_row($gTables['campi'], "codice", $form['campo_coltivazione']);
        if ($item_campi['id_colture'] > 0) {
    ...
    } else { //altrimenti carico solo il campo
		$item_campi = gaz_dbi_get_row($gTables['campi'], "codice", $form['campo_coltivazione']);
	}
    $form['search_partner'] = "";	
...
?>
    ...
    if (!empty($_POST['Insert'])) { // 	Se viene inviata la richiesta di conferma totale ...
        ...
        while ($row = $result->fetch_assoc()) {
            $fine_sosp = $row['giorno_decadimento'];
            $fine_sosp = strtotime($fine_sosp);
        ...
        for ($m = 0;$m <= $form['nmov'];++$m) {
            ...
            If ($itemart['good_or_service'] == 0 or $itemart['good_or_service'] == 2) { // se non è un servizio
                ...
                if (isset($_POST['Update'])) {
                    ...
                    if ($qta['artico']==$form['artico'][$m]){// Se l'articolo inserito nel form è lo stesso precedentemente memorizzato nel db, prendo la quantità precedentemente memorizzata e la riaggiungo alla giacenza di magazzino
...
<?php
            ...
            if ($toDo == "update" && $qta['artico']==$form['artico'][$m]) { // se è un update, e non è stato cambiato l'articolo tolgo il rame metallo memorizzato in precedenza
                $rame_met_annuo = $rame_met_annuo - $rame_metallo * gaz_format_quantity($form['quanti'][$m], 0, $admin_aziend['decimal_quantity']);
            }
            if (($form['quanti'][$m]>0 && $campo_coltivazione > 0) && ($dim_campo > 0) && ($rame_met_annuo + ($rame_metallo * gaz_format_quantity($form['quanti'][$m], 0, $admin_aziend['decimal_quantity'])) > (6 * $dim_campo))) {
        ...
        if (empty($msg)) { // nessun errore
            ...
            for ($form['mov'] = 0;$form['mov'] <= $form['nmov'];++$form['mov']) {
                if (!empty($form['artico'][$form['mov']])) {
                    ...
                    if ($form['operat'][$form['mov']] == 1) { // se il movimento è in entrata -carico-
...
$gForm->selectFromDB('campi', 'campo_coltivazione','codice', $form['campo_coltivazione'], 'codice', 1, ' - ','descri','TRUE','FacetSelect' , null, '');
if (isset($item_campi['ricarico'])){
	echo "Superficie: ", gaz_format_quantity($item_campi['ricarico'], 1, $admin_aziend['decimal_quantity']), " ha</td></tr>";
}
...
?>
		...
		<input type="hidden" name="operat<?php echo $l; ?>" value="<?php echo $form['operat'][$form['mov']]; ?>">
            ...
            if ((count($lm->available) > 1) && (intval($form['nmov']) == 0)) {
                foreach ($lm->available as $v_lm) {
                    if ($v_lm['id'] <> $form['id_lotmag'][$form['mov']]) {
                        echo '<div>change to:<button class="btn btn-xs btn-warning" type="image" onclick="this.form.submit();" name="id_lotmag' . $form['mov'] . '" value="' . $v_lm['id'] . '">' . $v_lm['id'] . ' lotto n.:' . $v_lm['identifier'] . ' scadenza:' . gaz_format_date($v_lm['expiry']) . ' - disponibili: ' . gaz_format_quantity($v_lm['quanti']) . '</button></div>';
                    }
                }
            } else {
