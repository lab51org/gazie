<?php
/*
  --------------------------------------------------------------------------
  GAzie - Gestione Azienda
  Copyright (C) 2004-2020 - Antonio De Vincentiis Montesilvano (PE)
  (http://www.devincentiis.it)
  <http://gazie.sourceforge.net>
  --------------------------------------------------------------------------
  Questo programma e` free software;   e` lecito redistribuirlo  e/o
  modificarlo secondo i  termini della Licenza Pubblica Generica GNU
  come e` pubblicata dalla Free Software Foundation; o la versione 2
  della licenza o (a propria scelta) una versione successiva.

  Questo programma  e` distribuito nella speranza  che sia utile, ma
  SENZA   ALCUNA GARANZIA; senza  neppure  la  garanzia implicita di
  NEGOZIABILITA` o di  APPLICABILITA` PER UN  PARTICOLARE SCOPO.  Si
  veda la Licenza Pubblica Generica GNU per avere maggiori dettagli.

  Ognuno dovrebbe avere   ricevuto una copia  della Licenza Pubblica
  Generica GNU insieme a   questo programma; in caso  contrario,  si
  scriva   alla   Free  Software Foundation, 51 Franklin Street,
  Fifth Floor Boston, MA 02110-1335 USA Stati Uniti.
  --------------------------------------------------------------------------
 */

namespace \GAzie\Interfacce\;

/**
 * Interface for invoice rows
 */
interface FatturaRow  {

	public function getFattura();
	
	public function setFattura(Fattura $fattura);

	public function getNumRow();

	public function setNumRow(int $numrow);

	public function getDescription();

	public function setDescription(string $description);

	public function getCodiceProdotto( );

	public function setCodiceProdotto($codice);

	public function getUm( );
	
	public function setUm( $um );

	public function getQuantity( );
	
	public function setQuantity(float $qta);

	public function getPrice( );
	
	public function setPrice(float $qta);

	public function getSconto( );

	public function setSconto(float $sconto );

	public function getProvvigione( );

	public function setProvvigione( float $provvigione );
	
	public function getImporto( );

	public function getIva();

	public function setIva(float $iva );

	public function getCodiceConto();

	public function setCodiceConto(string $conto );

}

