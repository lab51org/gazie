...
ALTER TABLE `gaz_XXXcontract` ADD COLUMN `id_assets` INT(9) NOT NULL DEFAULT '0' COMMENT 'Utilizzato in caso di contratto asssociato ad un bene strumentale che è soggetto a manutenzione periodica' AFTER `id_customer`, ADD INDEX `id_assets` (`id_assets`);
ALTER TABLE `gaz_XXXartico`	ADD COLUMN `id_assets` INT(9) NULL COMMENT 'Riferimento al bene strumentale ( id di gaz_NNNassets), quando maggiore di 0 è un bene strumentale e come tale in genere non in vendita ' AFTER `codice_fornitore`, ADD INDEX `id_assets` (`id_assets`);
