...
INSERT INTO `gaz_anagra` (ragso1, indspe, capspe, citspe, prospe, country,id_currency,id_language,telefo,pariva,codfis) SELECT ragione_sociale, indirizzo, cap, citta,provincia,'IT',1,1,telefo,partita_iva,codice_fiscale FROM `gaz_XXXvettor` WHERE `gaz_XXXvettor`.`id_anagra` = 0 AND `gaz_XXXvettor`.`partita_iva` > 0;
UPDATE `gaz_XXXvettor` t1 JOIN `gaz_anagra` t2 ON t1.partita_iva = t2.pariva SET t1.id_anagra = t2.id WHERE t1.partita_iva > 0;
