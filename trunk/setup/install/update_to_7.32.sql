...
ALTER TABLE `gaz_aziend` ADD COLUMN `update_ecommerce` TINYINT(1) NOT NULL DEFAULT '0' COMMENT 'indico 1 quando c\'è un ecommerce collegato che quindi deve essere aggiornato tramite API ad ogni cambiamento, es: nuovo movimento di magazzino, nuovo cliente, nuovo articolo,nuova categoria merceologica, nuova aliquota IVA e di controllare la presenza di nuovi ordini e/o clienti inseriti in esso' AFTER `web_url`;
