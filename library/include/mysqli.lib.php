...
function gaz_dbi_record_count($table, $where) {
   ...
   */
   /*
   $sql = "SELECT COUNT(*) AS num_rows FROM " . $table . (($where != "") ? " WHERE " . $where : "");
   $result = mysqli_query($link, $sql);
   $count = mysqli_fetch_array($result)['num_rows'];
   */
   $sql = "SHOW COLUMNS FROM ".$table;
   $result = mysqli_query($link,$sql);
   $first_column = mysqli_fetch_array($result)['Field'];
   $sql = "SELECT COUNT(".$first_column.") AS num_rows FROM " . $table . (($where != "") ? " WHERE " . $where : "");
   $result = mysqli_query($link, $sql);
   $count = mysqli_fetch_array($result)['num_rows'];
   return $count;
}
...
function movmagInsert($newValue) {
  $table = 'movmag';
  $columns = array('caumag', 'operat', 'datreg', 'tipdoc', 'desdoc', 'datdoc', 'clfoco', 'scochi', 'id_rif', 'artico', 'id_lotmag', 'id_orderman', 'quanti', 'prezzo', 'scorig', 'campo_coltivazione', 'status', 'adminid');
  $newValue['adminid'] = $_SESSION["user_name"];
  $last_id=tableInsert($table, $columns, $newValue);
...
function movmagUpdate($codice, $newValue) {
  $table = 'movmag';
  $columns = array('caumag', 'operat', 'datreg', 'tipdoc', 'desdoc', 'datdoc', 'clfoco', 'scochi', 'id_rif', 'artico', 'id_lotmag', 'id_ordeman', 'quanti', 'prezzo', 'scorig', 'campo_coltivazione', 'status', 'adminid');
  $newValue['adminid'] = $_SESSION["user_name"];
  tableUpdate($table, $columns, $codice, $newValue);
	// aggiorno l'e-commerce ove presente
	if (class_exists('APIeCommerce')){
