...
function submenu($array) {
	...
	for ($i=1; $i < 15; $i++) {
		if (array_key_exists($i, $array)) {
			...
			echo '<a href="' . $array[$i]['link'] . '">' . stripslashes ( $array[$i]['name'] ). '</a>';
			submenu($array[$i]);
			$numsub++;
		}
	}
    ...
    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
      <ul class="nav nav-tabs">
		...
		foreach ( $menuArray[0] as $menu ) {
			if ( $i > 4 ) {
				if ( count($menu)>5 ) {
					echo '<li class="dropdown"><a class="row-menu" href="'.$menu['link'].'">'.$menu['name'].'<span class="caret"></span></a>';
				} else {
					echo '<li><a class="row-menu" href="'.$menu['link'].'">'.$menu['name'].'</a>';
				}
				submenu($menu);
				$livello3 = $menu;
			}
