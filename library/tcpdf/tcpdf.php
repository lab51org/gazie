...
require('../../library/tcpdf/tcpdf_config.php');
 ...
 * unicode data
 */
require('../../library/tcpdf/unicode_data.php');

/**
 * html colors table
 */
require('../../library/tcpdf/htmlcolors.php');

/**
 * barcode class
 */
require("../../library/tcpdf/barcodes.php");
...
if (!class_exists('TCPDF')) {
     ...
     */
    define('PDF_PRODUCER','TCPDF 4.0.030 www.tcpdf.org patched for GAzie');
    ...
    */
    class TCPDF {
        ...
        public function setPageFormat($format, $orientation="P") {
            ...
            if (is_string($format)) {
                ...
                switch (strtoupper($format)) {
                    ...
                    case 'GAZIE': {$format = array(562.11,809.86); break;}
        ...
        */
        ...
        public function Header() {
            ...
            if (($headerdata['logo']) AND ($headerdata['logo'] != K_BLANK_IMAGE)) {
                //patched for GAzie
                $this->MemImage($headerdata['logo'], $this->GetX(), $this->getHeaderMargin(), $headerdata['logo_width'],'Logo aziendale');
        ...
        */
        public function AddFont($family, $style='', $file='') {
            ...
            require_once($this->_getfontpath().$file);
    ...
    protected function _parsemempng($var,$code)
    {
        $pos=0;
        //Check signature
        $sig = $this->_readstr($var,$pos, 8);
        if($sig != chr(137).'PNG'.chr(13).chr(10).chr(26).chr(10))
            $this->Error('Not a PNG image, a partire dalle versione 4.0.1, GAzie richiede che i file delle immagini del logo, degli articoli e delle categorie siano in formato PNG. <br />Aggiornare a questo formato &egrave; indispensabile per poter generare correttamente i file PDF dei documenti!</br> Clicca <a href="../../modules/root/convert_images.php" > QUI </a> fare la conversione in automatico.');
        //Read header chunk
        ...
        if($bpc>8)
            $this->Error('16-bit depth not supported: '.$code);
        ...
        elseif($ct==3)
            ...
            $this->Error('<br />Trasparenze non supportate<br />Alpha channel not supported:<br />su '.$code);
        if(ord($this->_readstr($var,$pos,1))!=0)
            $this->Error('<br />Metodo di compressione sconosciuto<br />Unknown compression method:<br />su '.$code);
        if(ord($this->_readstr($var,$pos,1))!=0)
            $this->Error('<br />Metodo di filtro sconosciuto<br />Unknown filter method:<br />su '.$code);
        if(ord($this->_readstr($var,$pos,1))!=0)
              $this->Error('<br />Interallacciamento non supportato<br /> Interlacing not supported<br />su:'.$code);
        $this->_readstr($var,$pos,4);
        $parms='/DecodeParms <</Predictor 15 /Colors '.($ct==2 ? 3 : 1).' /BitsPerComponent '.$bpc.' /Columns '.$w.'>>';
        //Scan chunks looking for palette, transparency and image data
        $pal='';
    ...
    /* PUBLIC FUNCTIONS */
    /********************/
    public function MemImage($data, $x, $y, $w=0, $h=0, $link='')
    {
        //Put the PNG image stored in $data
        $id = md5($data);
        if(!isset($this->images[$id]))
        {
            $info = $this->_parsemempng( $data,$link );
            $info['i'] = count($this->images)+1;
            $this->images[$id]=$info;
        }
        else
